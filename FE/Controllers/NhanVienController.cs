...
//                if ((DateTime.Now - entity.NamSinh).TotalDays / 365 < 18 || (DateTime.Now - entity.NamSinh).TotalDays / 365 <= 0)
//                {
...
//                        ModelState.AddModelError(nameof(entity.CCCD), "CCCD phải có đúng 12 chữ số.");
//                    }
//                    else if (allEmployees.Any(e => e.ID_Nhan_Vien != id && e.CCCD == entity.CCCD))
//                    {
//                        ModelState.AddModelError(nameof(entity.CCCD), "CCCD đã tồn tại.");
//                    }
//                }
...
using System.Text.RegularExpressions;
using System.Threading.Tasks;
using System.Globalization; // Để tính tuổi chính xác
...
using Microsoft.Extensions.Logging;
using Service.IService;
using System.DirectoryServices; // Nếu cần, nhưng không dùng
    ...
    public class NhanVienController : Controller
    {
        ...
        public async Task<IActionResult> Create(NhanVien entity, IFormFile anhNhanVien)
        {
            try
            {
                ...
                // Check Ho_Ten: Không trống, trim, và optional remove diacritics nếu cần
                entity.Ho_Ten = entity.Ho_Ten?.Trim();
                if (string.IsNullOrWhiteSpace(entity.Ho_Ten))
                {
                    ModelState.AddModelError(nameof(entity.Ho_Ten), "Họ tên không được để trống.");
                }
                // Optional: entity.Ho_Ten = RemoveDiacritics(entity.Ho_Ten); // Nếu muốn chuẩn hóa

                // Check Email: Valid và unique (nếu có)
                ...
                // Check So_Dien_Thoai: 10-11 digits, bắt đầu 0 (VN), unique (nếu có)
                entity.So_Dien_Thoai = entity.So_Dien_Thoai?.Trim();
                if (!string.IsNullOrWhiteSpace(entity.So_Dien_Thoai))
                {
                    if (!IsValidPhone(entity.So_Dien_Thoai))
                    {
                        ModelState.AddModelError(nameof(entity.So_Dien_Thoai), "Số điện thoại phải có 10-11 chữ số và bắt đầu bằng 0.");
                    }
                    else if (allEmployees.Any(e => e.So_Dien_Thoai == entity.So_Dien_Thoai))
                    {
                        ModelState.AddModelError(nameof(entity.So_Dien_Thoai), "Số điện thoại đã tồn tại.");
                ...
                // Check CCCD: 12 digits, unique (nếu có)
                entity.CCCD = entity.CCCD?.Trim();
                if (!string.IsNullOrWhiteSpace(entity.CCCD))
                {
                    if (!IsValidCCCD(entity.CCCD))
                    {
                ...
                // Check tuổi: Tính chính xác hơn (xử lý năm nhuận, ngày/tháng)
                if (!IsValidAge(entity.NamSinh))
                {
                    ModelState.AddModelError(nameof(entity.NamSinh), "Nhân viên phải đủ 18 tuổi.");
                }

                // Upload ảnh (giữ nguyên, nhưng thêm log nếu lỗi)
                if (anhNhanVien != null && anhNhanVien.Length > 0)
                {
                    var uploadResult = await TrySaveImageAsync(anhNhanVien);
                    if (!uploadResult.Success)
                ...
                TempData["SuccessMessage"] = $"Thêm nhân viên {entity.Ho_Ten} thành công."; // Sửa khớp với cshtml
                return RedirectToAction("Index", "NhanVien");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Lỗi khi thêm nhân viên.");
                TempData["ErrorMessage"] = $"Lỗi khi thêm nhân viên: {ex.Message}"; // Sửa khớp
                return View(entity);
            }
        }
        #endregion
        ...
        public async Task<IActionResult> Edit(int id, NhanVien entity, IFormFile anhNhanVien)
        {
            try
            {
                ...
                // Tương tự Create, nhưng skip unique nếu không thay đổi
                entity.Ho_Ten = entity.Ho_Ten?.Trim();
                if (string.IsNullOrWhiteSpace(entity.Ho_Ten))
                {
                    ModelState.AddModelError(nameof(entity.Ho_Ten), "Họ tên không được để trống.");
                ...
                if (!string.IsNullOrWhiteSpace(entity.Email))
                {
                    ...
                    else if (allEmployees.Any(e => e.ID_Nhan_Vien != id && string.Equals(e.Email, entity.Email, StringComparison.OrdinalIgnoreCase)))
                    {
                        ModelState.AddModelError(nameof(entity.Email), "Email đã tồn tại.");
                    }
                }
                ...
                if (!IsValidAge(entity.NamSinh))
                {
                    ModelState.AddModelError(nameof(entity.NamSinh), "Nhân viên phải đủ 18 tuổi và ngày sinh phải hợp lệ (không ở tương lai).");
                }
                ...
                // Xác định fields thay đổi (giữ nguyên)
                var fieldsChanged = new List<string>();
                if (entity.Ho_Ten != currentEmployee.Ho_Ten) fieldsChanged.Add("Họ tên");
                if (entity.Email != currentEmployee.Email) fieldsChanged.Add("Email");
                if (entity.So_Dien_Thoai != currentEmployee.So_Dien_Thoai) fieldsChanged.Add("Số điện thoại");
                ...
                TempData["SuccessMessage"] = changedMessage; // Sửa khớp
                return RedirectToAction(nameof(Index));
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Lỗi khi cập nhật nhân viên.");
                TempData["ErrorMessage"] = $"Lỗi khi cập nhật nhân viên: {ex.Message}"; // Sửa khớp
                return View(entity);
            }
        }
        #endregion

        // ... (các region khác giữ nguyên)
        ...
        private bool IsValidEmail(string email)
        {
            ...
            try
            {
                ...
                return addr.Address == email && email.Contains("."); // Thêm check basic domain
            }
            catch
            {
                return false;
        ...
        {
            if (string.IsNullOrWhiteSpace(phone)) return false;
            var pattern = @"^0\d{9,10}$"; // Bắt đầu bằng 0, 10-11 digits
        ...
        {
            if (birthDate > DateTime.Now) return false; // Không ở tương lai
        ...
        private async Task<(bool Success, string ErrorMessage, string RelativePath)> TrySaveImageAsync(IFormFile file)
        {
            try
            {
                ...
                // Tên file an toàn hơn: loại bỏ ký tự đặc biệt
                var originalFileName = Path.GetFileNameWithoutExtension(file.FileName).Replace(" ", "_").Replace("[^a-zA-Z0-9_-]", "");
                var safeFileName = $"{DateTime.UtcNow:yyyyMMddHHmmss}_{Guid.NewGuid().ToString("N").Substring(0, 8)}_{originalFileName}{extension}";
        ...
        private string RemoveDiacritics(string text) // Giữ nguyên hàm của bạn
        {
            if (string.IsNullOrEmpty(text)) return text;
